...
export default function ProfilePage() {
  ...
  async function handleBioSubmit(e) {
    ...
    try {
      ...
      if (res.ok) {
        setBioMessage({ type: 'success', text: "Your information was updated successfully" });
        refetch();
      } else {
        setBioMessage({ type: 'error', text: "There was an error updating your information" });
      }
    } catch (error) {
      setBioMessage({ type: 'error', text: "There was an error updating your information" });
    } finally {
  ...
  async function handleClubSubmit(e) {
    ...
    try {
      ...
      if (res.ok) {
        setClubMessage({ type: 'success', text: "Club profile updated successfully" });
        refetch();
      } else {
        setClubMessage({ type: 'error', text: "Failed to update club profile" });
      }
    } catch (error) {
      setClubMessage({ type: 'error', text: "Failed to update club profile" });
    } finally {
  ...
  async function handleBookTime(timeId) {
    ...
    try {
      ...
      if (res.ok) {
        setSchedulingMessage({ type: 'success', text: "Time slot booked successfully" });
        setBookingLocation('');
        fetchSchedulingData();
        refetch();
      } else {
        const data = await res.json();
        setSchedulingMessage({ type: 'error', text: data.error || "Failed to book time slot" });
      }
    } catch (error) {
      setSchedulingMessage({ type: 'error', text: "Failed to book time slot" });
    } finally {
      setBooking(false);
    }
  }
  ...
  function getTabs() {
    ...
    if (user?.is_staph) {
      tabs.push({ id: 'photographerTools', label: "Staph Tools" });
    }

    if (user?.role === 'club') {
      tabs.push({ id: 'club', label: "Club Info" });
            ...
            <div className="space-y-6">
              ...
              {user?.role === 'staph' && (
                <div className="card-elevated p-6">
                  ...
                  <p className="text-sm text-text-secondary mb-4">{"Keep your information up to date. This data can be used to autofill your Senior Bio when you become a senior."}</p>
                  ...
                  {bioLoading ? (
                    ...
                    <form onSubmit={handleBioSubmit} className="space-y-4">
                      ...
                      >
                        {bioSaving ? "Saving..." : "Save Information"}
                      </Button>
                    </form>
                  )}
                </div>
              ...
              {user?.role === 'staph' && (
                <div className="flex items-center justify-between px-4 py-3 bg-bg-secondary/50 rounded-lg">
                  <span className="text-sm text-text-secondary">{"Senior Bio"}</span>
                  <button
          ...
          {activeTab === 'club' && user?.role === 'club' && (
            <div>
              ...
              <form onSubmit={handleClubSubmit} className="card-elevated p-6 space-y-4">
                ...
                >
                  {clubSaving ? "Saving..." : "Save Changes"}
                </Button>
              </form>
            </div>
          )}
                ...
                <div>
                  ...
                  <div className="mb-4">
                    <TextField
                      ...
                      helperText={"Enter where the photoshoot will take place"}
                    />
                  </div>
                  ...
                  {schedulingLoading ? (
                    ...
                    <p className="text-text-secondary">{"No available time slots"}</p>
                  ) : (
                    <div className="space-y-3">
