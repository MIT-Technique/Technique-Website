...
export default function BioPage() {
  ...
  const [isFrozen, setIsFrozen] = useState(false); // null = loading, true = frozen, false = open
    ...
    (async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (json.data.email) setEmail(json.data.email.trim().toLowerCase());
        ...
        try {
          const photoRes = await fetch(`/api/senior-photos`, {
            cache: "no-store",
          });
          if (photoRes.ok) {
  ...
  function validateEmail(value) {
    ...
    if (!normalized.endsWith("@mit.edu")) {
      setEmailError("Must be a valid @mit.edu email address");
      return false;
    }
    setEmailError("");
    return true;
  ...
  async function updateBio() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setSnackMessage(data.error || "There was an error saving your bio");
        setOpen(true);
        setError(true);
      } else {
        setSnackMessage("Your bio was saved successfully");
        setOpen(true);
        setError(false);
      }
    } catch {
      setSnackMessage("There was an error saving your bio");
      setError(true);
      setOpen(true);
    }
  }
    ...
    <>
          ...
          {isFrozen === null ? (
            ...
            <div className="card-elevated flex flex-col items-center text-center py-10 px-6">
              <svg
                ...
                className="w-5 h-5 text-[#750014] mb-4"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1}
            ...
            >
              ...
              <TextField
                ...
                error={!!emailError}
                helperText={
                  emailError || "Enter first to autofill saved responses"
                }
                sx={textFieldSx}
                fullWidth
                InputProps={{
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <TextField
                      ...
                      fullWidth
                      placeholder={
                        "Enter a quote for your yearbook entry (optional)"
                      }
                    />
                    ...
                    <TextField
                      ...
                      onChange={(event) =>
                        setExtracurriculars(event.target.value)
                      }
                  ...
                  <div className="mt-4 mb-2">
                    ...
                    <FormControl fullWidth sx={{ mb: 2 }}>
                      ...
                      <Select
                        ...
                        displayEmpty
                        onChange={(event) =>
                          setPhotoPreference(event.target.value)
                        }
                        sx={selectSx}
                      >
                        <MenuItem value="" disabled>
                        ...
                        <MenuItem value="technique_photo">
                          {"I would like to use the Technique studio photo"}
                        </MenuItem>
                        <MenuItem value="uploaded_photo">
                          {"I would like to upload my own photo"}
                        </MenuItem>
                        <MenuItem value="no_image">
                          {"I do not want an image in the yearbook"}
                        </MenuItem>
                      </Select>
                    </FormControl>
                    ...
                    {photoPreference === "uploaded_photo" && (
                      <>
                        <p className="text-xs text-text-muted mb-3">
                          {
                            "For best results, use a portrait-oriented photo with letter-size proportions (8.5 x 11)."
                          }{" "}
                          <a
                            href="/seniors"
                            className="text-primary hover:underline"
      ...
      >
        ...
        >
          {snackMessage ||
            (error
              ? "There was an error saving your bio"
              : "Your bio was saved successfully")}
        </Alert>
      </Snackbar>
      <Footer />
    </>
