  ...
  async function handleRemoveManualMember(memberId) {
    ...
    try {
      ...
      if (res.ok) {
        setMembersMessage({
          type: 'success',
          text: memberName ? `${memberName} removed` : "Member removed successfully"
        });
  ...
  async function handleUpdateMemberRole(memberId, newRole) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setMembersMessage({ type: 'error', text: data.error || "Failed to update role" });
      }
    } catch (error) {
      setMembersMessage({ type: 'error', text: "Failed to update role" });
    } finally {
    ...
    <>
      <main className="min-h-screen pt-24 lg:pt-32 pb-12">
        <section className="container-text">
          <h1 className="mb-2">{"Club Dashboard"}</h1>
            ...
            <div>
              {isFrozen && (
                <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  <p className="text-gray-700 font-medium">{"This form is currently closed. You cannot make changes at this time."}</p>
              ...
              <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-text-muted mb-2">{"Briefly describe your club's purpose and activities."}</p>
                  <textarea
                    value={formData.description}
                    onChange={handleDescriptionChange}
                    onBlur={handleDescriptionBlur}
                    placeholder={"MIT Technique is MIT's yearbook club. We publish annual printed yearbooks. Our club is involved in many activities including photography, layout, graphic design, and more!"}
              ...
              {/* Images Section */}
              <div className="mt-8">
                  <label className="block text-sm font-medium mb-2">{"Candid Images"}</label>
                  <p className="text-sm text-text-muted mb-4">{"Upload up to 3 candid images of your club (1 main + 2 optional). Max 20MB each."}</p>
                  {imageMessage.text && (
                    ...
                    {[1, 2, 3].map((slot) => {
                      ...
                      return (
                        <ImageUpload
                          ...
                          label={slot === 1 ? "Main Image" : "Additional (Optional)"}
          ...
          {activeTab === 'members' && (
            <div>
              ...
              <h2 className="text-lg font-medium mb-4">{"Club Members"}</h2>

              {manualMembers.length > 30 && (
                <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                  <p className="text-sm text-amber-800 font-medium">{"Your club has over 30 members — please assign officer positions!"}</p>
                  <p className="text-xs text-amber-700 mt-1">{"Use the role field next to each member's name to designate officers (President, Vice President, Secretary, etc.)."}</p>
                </div>
              )}
