...
export default function LivingGroupPage() {
  ...
  async function handleBook(timeId, locations) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({ type: 'error', text: data.error || "Failed to book time slot" });
      }
    } catch (error) {
      setMessage({ type: 'error', text: "Failed to book time slot" });
    } finally {
      setBooking(false);
    }
  }
  ...
  async function handleAddSingleMember(e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewMemberSection('');
        setMembersMessage({ type: 'success', text: "Member added successfully" });
        fetchMembers();
      } else {
        const data = await res.json();
  ...
  async function handleImportBulk() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        let message = `Added ${data.count} members successfully`;
        if (data.parseErrors?.length > 0) {
          message += ` (${data.parseErrors.length} errors)`;
        }
        if (data.duplicates?.length > 0) {
          message += ` (${data.duplicates.length} duplicates skipped)`;
        }
        ...
        setMembersMessage({
          type: 'error',
          text: data.error || "Failed to import members",
        });
      }
    } catch (error) {
  ...
  async function handleBulkReset() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMembersMessage({
          type: 'success',
          text: `${deletedCount} members removed from ${sectionText}`,
        });
      } else {
        const data = await res.json();
        setMembersMessage({ type: 'error', text: data.error || 'Failed to remove members' });
      }
    } catch (error) {
      console.error('Bulk reset error:', error);
      setMembersMessage({ type: 'error', text: 'Failed to remove members' });
    } finally {
  ...
  async function handleRevertBulkReset() {
    ...
    try {
      ...
      if (allSuccess) {
        ...
        setMembersMessage({
          type: 'success',
          text: 'Members restored successfully',
        });
        setDeletedMembersForRevert(null);
      } else {
        setMembersMessage({ type: 'error', text: 'Failed to restore members' });
      }
    } catch (error) {
  ...
  async function handleUpdateMemberSection(memberId, sectionName) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setMembersMessage({ type: 'error', text: data.error || "Failed to update member" });
      }
    } catch (error) {
      setMembersMessage({ type: 'error', text: "Failed to update member" });
    } finally {
  ...
  async function handleAddSection(e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSections(data.sections || []);
        setNewSectionName('');
        setSectionsMessage({ type: 'success', text: "Section added successfully" });
      } else {
        const data = await res.json();
        setSectionsMessage({ type: 'error', text: data.error || "Failed to add section" });
      }
    } catch (error) {
      setSectionsMessage({ type: 'error', text: "Failed to add section" });
    } finally {
  ...
  async function confirmRemoveSection() {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setSections(data.sections || []);
        setSectionsMessage({ type: 'success', text: "Section removed successfully" });
        fetchMembers(); // Refresh members since their sections might have been cleared
      } else {
        const data = await res.json();
        setSectionsMessage({ type: 'error', text: data.error || "Failed to remove section" });
      }
    } catch (error) {
      setSectionsMessage({ type: 'error', text: "Failed to remove section" });
    } finally {
  ...
  async function handleAssignSection(photoshootTimeId, slotStart, slotEnd, sectionName) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSectionsMessage({ type: 'success', text: "Assignment saved" });
      } else {
        const data = await res.json();
        setSectionsMessage({ type: 'error', text: data.error || "Failed to save assignment" });
      }
    } catch (error) {
      setSectionsMessage({ type: 'error', text: "Failed to save assignment" });
    } finally {
      setSavingSlot(null);
    }
  }
  ...
  async function handleCancelProposal(proposalId) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setMessage({ type: 'error', text: data.error || "Failed to cancel proposal" });
      }
    } catch (error) {
      setMessage({ type: 'error', text: "Failed to cancel proposal" });
    } finally {
  ...
  const tabs = [
    // Dorms get Sections tab, FSILGs get Profile tab
    {
      ...
      label: isFsilg ? "Profile" : getSectionsLabel()
    },
  ...
  const scheduleTabs = [
    { id: 'book', label: "Book" },
    ...(!isFsilg && livingGroup?.dorm_sections?.length > 0 ? [{ id: 'assign', label: "Assign" }] : []),
  ];
    ...
    <>
      <main className="min-h-screen pt-24 lg:pt-32 pb-12">
        <section className="container-text">
          <h1 className="mb-2">{"Living Group Dashboard"}</h1>
          <p className="text-text-secondary mb-8">
            {`Welcome, ${livingGroup?.name || user?.email}`}
          </p>

          {/* Disabled Notice */}
          {isDisabled && (
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-red-600 font-medium">{"Your account has been disabled"}</p>
              <p className="text-sm text-red-500 mt-1">{"Please contact an administrator for assistance"}</p>
            </div>
          )}
          ...
          {isFrozen && !isDisabled && activeTab === 'schedule' && scheduleSubTab === 'book' && (
            <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
              <p className="text-gray-700 font-medium">{"Booking is currently closed. You cannot make changes at this time."}</p>
          ...
          {activeTab === 'profile' && !isFsilg && (
            <div>
              {/* Email Section */}
              <div className="mb-6">
                <label className="block text-sm font-medium mb-2">{"Display Email"}</label>
                <div className="flex gap-2">
                  <input
                    type="email"
                    value={livingGroupEmail}
                    onChange={(e) => setLivingGroupEmail(e.target.value)}
                    placeholder={"your-living-group@mit.edu"}
          ...
          {activeTab === 'schedule' && (
            <>
              ...
              {bookedTimes.length > 0 && (
                <div className="mb-8">
                  <h2 className="text-lg font-medium mb-4">{"Upcoming Photoshoots"}</h2>
                            ...
                            <div>
                              ...
                              {isPending && (
                                <p className="text-yellow-700 text-sm mt-1 font-medium">
                                  {"Pending — awaiting location confirmation"}
                                </p>
                              )}
                              {isPending && bookedTime.proposed_locations?.length > 0 && (
                                <p className="text-text-muted text-xs mt-0.5">
                                  {"Your proposed locations"}: {bookedTime.proposed_locations.join(', ')}
                                </p>
                              )}
                              {isConfirmed && bookedTime.location && (
                                <p className="text-green-700 text-sm mt-1">
                                  {"Confirmed location"}: {bookedTime.location}
                                </p>
                              )}
                            </div>
                            {!isDisabled && !isFrozen && (
                ...
                <div className="bg-white border border-border rounded-lg p-6">
                  ...
                  {bookViewMode === 'calendar' ? (
                    <CalendarView
                      ...
                      onPropose={async (proposalData) => {
                        ...
                        const data = await res.json();
                        if (res.ok) {
                          setMessage({ type: 'success', text: "Time proposed and posted successfully" });
                          if (data.proposal) {
                    ...
                    <>
                      {loading ? (
                        ...
                        <p className="text-text-secondary text-sm">{"No available time slots"}</p>
                      ) : (
                              ...
                              >
                                ...
                                {isBookingThis && (
                                  <div className="mt-3 pt-3 border-t border-border/50" onClick={(e) => e.stopPropagation()}>
                                    <p className="text-sm font-medium mb-2">{"Proposed Location(s)"}</p>
                                    <p className="text-xs text-text-muted mb-2">{"Enter one or more location options for the photoshoot. Technique will confirm the final location."}</p>
                                    <div className="space-y-2">
                        ...
                        <div className="mt-4">
                          {showListProposeForm ? (
                            <div className="border border-border rounded-lg p-4">
                              <h4 className="text-sm font-medium mb-3">{"Propose a Time"}</h4>
                              <p className="text-xs text-text-muted mb-3">{"Suggest a time for staph to accept. If staph accepts, your photoshoot will be scheduled."}</p>
                              <form
                                onSubmit={async (e) => {
                                  e.preventDefault();
                                  const form = e.target;
                                  ...
                                  try {
                                    ...
                                    setMessage({ type: 'error', text: "Failed to submit proposal" });
                                  } finally {
                                    setProposing(false);
                                  }
                                }}
                  ...
                  {proposals.filter(p => !hiddenProposals.has(p.id) && !(p.status === 'cancelled' && !p.accepted_by)).length > 0 && (
                    <div className="mt-6 pt-4 border-t border-border">
                      <h4 className="text-xs font-medium text-text-muted uppercase mb-2">{"Your Proposals"}</h4>
                      <div className="space-y-2">
                          ...
                          >
                            <div className="flex justify-between items-center">
                              <div>
                                ...
                                {proposal.status === 'accepted' && proposal.accepter && (
                                  <p className="text-green-600 text-xs mt-1">
                                    {`Accepted by ${proposal.accepter.name || proposal.accepter.email}`}
                                  </p>
                                )}
                              </div>
                              {(proposal.status === 'pending' || proposal.status === 'accepted') && !isFrozen && (
                                ...
                                >
                                  {cancellingProposalId === proposal.id ? '...' : confirmCancelProposalId === proposal.id ? "Confirm?" : "Cancel"}
                                </button>
                              )}
                            </div>
                          </div>
                ...
                <div>
                  {bookedTimes.length === 0 ? (
                    <div className="py-8 text-center">
                      <p className="text-text-secondary text-sm mb-3">{"No upcoming photoshoots booked yet. Book a photoshoot time to start assigning sections."}</p>
                      <button
                        onClick={() => setScheduleSubTab('book')}
                        className="text-sm text-red-600 hover:text-red-700 underline"
                      >
                        {"Go to Book Photoshoot"}
                      </button>
                    </div>
                  ) : (
                    <div>
                      <h3 className="text-lg font-medium mb-2">{"Assign Sections to Time Slots"}</h3>
                      <p className="text-text-secondary text-sm mb-4">
                        {"Assign each section to a 15-minute time slot during your photoshoot."}
                      </p>
                      ...
                      {(() => {
                          ...
                          return (
                            <div className="mb-6 p-3 bg-yellow-50 border border-yellow-200 rounded">
                              <p className="text-sm text-yellow-800">
                                {unassigned.length > 0 && (
                                  <><span className="font-medium">{"Sections not yet assigned"}:</span>{' '}{unassigned.join(', ')}</>
                                )}
                        ...
                        return (
                          <div className="mb-6 p-3 bg-green-50 border border-green-200 rounded">
                            <p className="text-sm p-0 font-medium text-green-800">{"All sections assigned!"}</p>
                          </div>
                        );
                      })()}
          ...
          {activeTab === 'sections' && (
            <div>
              <p className="text-text-secondary text-sm mb-4">{"Create sections to organize members (e.g., floors, wings, buildings). Members can then be assigned to sections."}</p>
              ...
              <form onSubmit={handleAddSection} className="mb-4 flex gap-2">
                <input
                  ...
                  placeholder={"Enter section name (e.g., 1st Floor)"}
                  className="flex-1 border border-border rounded px-4 py-2"
                />
                <button
                  type="submit"
                ...
                >
                  {addingSection ? "Adding..." : <><span className="sm:hidden text-lg leading-none">+</span><span className="hidden sm:inline">{"Add Section"}</span></>}
                </button>
              </form>
              ...
              {sectionsLoading ? (
                ...
                <div>
                  <p className="text-text-secondary text-sm mb-4">{"You can upload a single group photo for your living group, or create sections below to organize by floor/wing."}</p>
                  <div className="px-3 py-3 border border-border rounded-lg flex items-start gap-3 max-w-sm">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-text-muted mt-1">{"Upload one photo representing your entire living group. If you add sections later, this image will move to your first section."}</p>
                    </div>
                  </div>
                </div>
              ) : (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-2">
                            ...
                            >
                              {removingSectionName === section ? "Removing..." : "Remove"}
                            </button>
                          </div>
                          <button
                            onClick={() => {
              ...
              {sectionToRemove && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                    <h3 className="text-lg font-medium mb-2">{"Remove Section?"}</h3>
                    <p className="text-text-secondary mb-4">
                      {`Are you sure you want to remove '${sectionToRemove}'? All members in this section will be moved to 'Unassigned'.`}
                    </p>
                    <div className="flex gap-2 justify-end">
                      ...
                      >
                        {removingSectionName === sectionToRemove ? "Removing..." : "Confirm"}
                      </button>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === 'profile' && isFsilg && (
            <div>
              ...
              <div className="mb-8">
                ...
                <p className="text-text-secondary text-sm mb-3">{"Write a brief description of your organization (75 words max). This will be considered for inclusion in the yearbook."}</p>
                <textarea
                  value={description}
                ...
                <p className="text-xs text-text-muted mt-1 flex items-center gap-2">
                  <span className={description.trim().split(/\s+/).filter(Boolean).length > 75 ? 'text-red-600' : ''}>
                    {description.trim().split(/\s+/).filter(Boolean).length} / 75 {"words"}
                  </span>
                ...
                {[1, 2, 3].map((slot) => {
                  ...
                  return (
                    <ImageUpload
                      key={slot}
                      imageUrl={currentUrl}
                      label={slot === 1 ? "Main Photo" : "Additional (Optional)"}
              ...
              <p className="text-text-secondary text-sm mt-6 p-3 bg-gray-50 border border-gray-200 rounded">
                {"Note: Additional photos will be considered but are not guaranteed to be included if yearbook spacing is tight. Focus on getting the best single photo of everyone in your organization for the main image."}
              </p>
            </div>
          )}
            ...
            <div>
                ...
                <div className="mb-6">
                  <div className="mb-2">
                    ...
                    <p className="text-xs text-text-muted mb-2">
                      {"Paste names separated by commas, newlines, or tabs. Formats: 'First Last', 'Last, First', or single names."}
                    </p>
                  </div>
                  <textarea
                    value={bulkText}
                    onChange={(e) => setBulkText(e.target.value)}
                    placeholder={"John Smith, Jane Doe, Bob Johnson\nor\nJohn Smith\nJane Doe\nBob Johnson\nor\nSmith, John\nDoe, Jane"}
                  ...
                  <div className="mt-2 flex gap-2 flex-wrap">
                    {sections.length > 0 && (
                      <select
                        ...
                        className="border border-border rounded px-4 py-2"
                      >
                        <option value="">{"Assign sections later"}</option>
                        {sections.map((section) => (
                          <option key={section} value={section}>{section}</option>
                        ))}
                      </select>
                  ...
                  {showPreview && parsePreview && (
                    <div className="mt-4 p-4 border border-border rounded bg-gray-50">
                      ...
                      <p className="text-sm text-text-secondary mb-3">
                        {`${parsePreview.success.length} names will be imported`}
                      </p>

                      {parsePreview.success.length > 0 && (
                        <div className="mb-3">
                          <p className="text-xs font-medium text-green-600 mb-1">
                            {"Successfully parsed"} ({parsePreview.success.length})
                          </p>
                      ...
                      {parsePreview.errors.length > 0 && (
                        <div className="mb-3">
                          <p className="text-xs font-medium text-red-600 mb-1">
                            {"Could not parse"} ({parsePreview.errors.length})
                          </p>
                          <div className="max-h-40 overflow-y-auto bg-white p-2 rounded border text-sm">
              ...
              {membersLoading ? (
                ...
                if (filteredMembers.length === 0) {
                  return <p className="text-text-secondary">{"No members found" || 'No members found'}</p>;
                }
              ...
              {manualMembers.length > 0 && (
                <div className="mt-8 pt-6 border-t border-border">
                  <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                    ...
                    >
                      {bulkResetting ? "Resetting..." : "Reset Members"}
                    </button>
              ...
              {memberToRemove && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                  <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                    <h3 className="text-lg font-medium mb-2">{"Remove"}</h3>
                    <p className="text-text-secondary mb-4">
                      {"Remove this member?"}
                      <br />
              ...
              <ConfirmationModal
                ...
                message={
                  isFsilg || bulkResetSection === 'all'
                    ? `This will permanently delete all ${manualMembers.length} members. This action cannot be undone.`
                    : `This will permanently delete all ${manualMembers.filter(m => m.section_name === bulkResetSection).length} members from '${bulkResetSection}'. This action cannot be undone.`
                }
          ...
          {activeTab === 'settings' && (
            <div>
              ...
              <div className="mb-8">
                ...
                <p className="text-text-secondary text-sm mb-3">{"This email is used for password resets and contact purposes."}</p>
                <div className="flex gap-2">
                  <input
                    type="email"
                    value={livingGroupEmail}
              ...
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <h3 className="text-sm font-medium">{"Internal Documents"}</h3>
                ...
                <p className="text-text-secondary text-sm mb-4">{"Store internal links and notes for your organization. This information is only visible to your account."}</p>
