...
const PHOTOGRAPHER_STATUS_LABELS = {
  pending: "Pending",
  claimed: "Claimed",
  completed: "Completed",
  cancelled: "Cancelled",
    ...
    <>
      <main className="min-h-screen pt-24 lg:pt-32">
        ...
        <section className="section-tight container-text text-center">
          ...
          <p className="text-lg text-text-secondary font-light">{"If you need photography for an event, our community of MIT photographers can help."}</p>
        </section>
        ...
        <section className="section container-text text-center">
          <p className="mb-4">{"We offer event photography services for all organizations and groups affiliated with MIT. Technique matches you with one of our trained staph photographers to capture every moment during your event."}</p>
        </section>
      </main>
      <Footer />
    </>
    ...
    return (
      <div className="max-w-md mx-auto text-center">
        ...
        <h2 className="text-2xl font-light tracking-tight mb-2">
          {confirmation.updated ? "Request Updated!" : "Request Submitted!"}
        </h2>

        <p className="text-xs uppercase tracking-widest text-text-muted mt-8 mb-3">{"Your Confirmation Code"}</p>
        ...
        <p className="text-sm text-text-secondary font-light pb-1">
          {`${confirmation.hourlyRate}/hr x ${confirmation.durationHours} hrs = $${confirmation.totalCost.toFixed(2)}`}
        </p>

        <p className="text-xs text-text-muted font-light mb-8">{"Save this code to update your request later."}</p>
    ...
    <div className="max-w-lg mx-auto">
      <div className="border border-border/70 rounded px-8 py-10 sm:px-10">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-xl font-light tracking-tight">
            {isUpdateMode ? "Update Existing Request" : "Request a Photographer"}
          </h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {isUpdateMode && (
            <div className="space-y-6 pb-6 mb-2 border-b border-border/50">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                  <label className={labelClass}>{"Request Email"}</label>
                  <input
                    type="email"
          ...
          <div>
            ...
            <input
              ...
              placeholder={"e.g., Kresge Auditorium"}
              className={inputClass}
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-x-6">
            ...
            <div>
              <label className={labelClass}>{"Start Time (EST)"}</label>
              <select
          ...
          {rate && durationHours > 0 && (
            <div className="flex items-baseline justify-between py-3 border-y border-border/40">
              <span className="text-[11px] uppercase tracking-widest text-text-muted">{"Estimated Cost"}</span>
          ...
          <div>
            <label className={labelClass}>{"Cost Object (Optional)"}</label>
            <input
              type="text"
              value={form.costObject}
              onChange={(e) => updateField("costObject", e.target.value)}
              placeholder={"e.g., cost center, WBS element, or internal order number"}
              className={inputClass}
            />
            <p className="text-[11px] text-text-muted mt-1">{"Providing this now saves you a step after photo delivery."}</p>
          </div>
      ...
      <p className="text-center mt-5">
        <button
          ...
          className="text-[11px] uppercase tracking-widest text-text-muted hover:text-accent transition-colors"
        >
          {isUpdateMode ? "Submit a new request instead" : "Have a confirmation code? Update your request"}
        </button>
      </p>
    </div>
  );
...
function PhotographerSignIn({ onSignIn }) {
  ...
  async function handleSignIn(e) {
    ...
    try {
      ...
      if (!res.ok || !data.authorized) {
        setError("This email is not authorized. Contact an admin.");
        return;
      }
    ...
    <div className="mt-8 max-w-xs mx-auto">
      <form onSubmit={handleSignIn} className="flex items-center gap-2">
        <input
          ...
          placeholder={"photographer@mit.edu"}
          autoFocus
...
function statusBadge(status) {
  ...
  return (
    <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-sm border ${styles[status] || "border-gray-300 bg-gray-50 text-gray-600"}`}>
      {PHOTOGRAPHER_STATUS_LABELS[status] ?? status}
    </span>
  );
}

function RequestCard({ request, showClaim, isOwned, expandedId, setExpandedId, claimingId, onClaim, completingId, onComplete, submittingLinkId, onSubmitLink, linkInputs, setLinkInputs, notes, setNotes }) {
          ...
          <div>
            ...
            {request.dropbox_link ? (
              ...
              <>
                ...
                <p className="text-[11px] text-text-muted">{"Submitting will deliver photos to the requester and request their cost object."}</p>
              </>
            )}
          </div>
...
function PhotographerPanel({ photographerEmail, onSignOut }) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex justify-between items-baseline mb-8">
        <h2 className="text-lg font-light tracking-wide text-text-primary">{"Photographer Dashboard"}</h2>
        <button
      ...
      {loading ? (
        ...
        <>
          ...
          {otherRequests.length > 0 && (
            <PaginatedSection
              title={"All Requests (Staph Overview)"}
              items={otherRequests}
              emptyText=""
              page={otherPage}
