...
export async function POST(request: NextRequest) {
  try {
    ...
    // ==================== LIVING GROUP SIGNIN ====================
      ...
      // Look up living group by name with user email in one query
      const { data: livingGroup, error: lgError } = await supabaseAdmin
        .from("living_groups")
        .select("id, name, user_id, users!living_groups_user_id_fkey(email)")
        .ilike("name", name.trim())
        .single();

      const lgUser = Array.isArray(livingGroup?.users) ? livingGroup.users[0] : livingGroup?.users;
      if (lgError || !livingGroup || !lgUser?.email) {
        return NextResponse.json(
    ...
    // Authenticate and fetch user record in parallel
    ...
    const [authResult, userResult] = await Promise.all([
      supabaseAuth.auth.signInWithPassword({ email: authEmail, password }),
      supabaseAdmin.from("users").select("*").eq("email", authEmail).single(),
    ]);
